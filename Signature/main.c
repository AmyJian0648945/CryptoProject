...
int main(void){
	...
	uint8_t tempEM2[sizeMessage] = {0};
	uint16_t tempEMB[sizeModulusB] = {0};
	uint8_t EMB[sizeModulusB*2] = {0};
	uint16_t tempResult[sizeModulusB] = {0};
	...
	uint16_t tempEM1[sizeModulusA] = {0};
	uint8_t tempEMA[sizeModulusA*2] = {0};
	uint16_t receivedMessageA[sizeModulusA] = {0};
	uint16_t receivedMessageB[sizeModulusB] = {0};
	uint16_t identityAVerified = 0;	
	uint8_t msgLengthEncrypt[2] = {0x01, 0x00};
	uint8_t msgLengthDecrypt[2] = {0x00, 0x80};
	...
	signatureMessage(messageB, tempEM2);
	
	signMessage(tempEM2, receivedMessageB, modulusB, privateExponentB, modLength*4, sizeModulusB, sizePrExpB);
	printf("B sends g^y mod p and Sb(g^y mod p || g^x mod p) to A\n");
	
	// encryptMessage(tempEMB, EMB, sizeModulusB, K1, keySize);
	// printArray8(EMB, "EMB",sizeModulusB*2);
	
	// decryptMessage(EMB, tempResult, sizeModulusB*2, embLength, K2, keySize);
	...
	signMessage(encodedMessageA, receivedMessageA, modulusA, privateExponentA, sizeMessage,sizeModulusA, sizePrExpA);
	printf("A sends Sa(g^x mod p || g^y mod p)\n");
	// encryptMessage(tempEM, tempEMA, sizeModulusA, msgLengthEncrypt, K2, keySize);
	// printArray8(tempEMA,"encryptedMessage", sizeModulusA*2);
	// decryptMessage(tempEMA, tempEM, sizeModulusA*2, msgLengthDecrypt, K2, keySize);
	// printArray16(tempEM, "decryptedMessage", sizeModulusA);
