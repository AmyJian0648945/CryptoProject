...
//size32_t bufPos, read;
//size16_t input_buffer[BUFFERSIZE];
//size16_t output_buffer[BUFFERSIZE];
//size16_t buffers[BUFFERSIZE*BUFFERS]; // big array used to store a large number of samples that will be processed buffer by buffer

size16_t pCompressed_in[PACKSIZE];
size16_t pCompressed_out[PACKSIZE];

struct FirstPhaseChunk chunk1_L;
struct FirstPhaseChunk chunk1_R;

struct SecondPhaseChunk chunk2_L;
struct SecondPhaseChunk chunk2_R;
...
void init_audio(){
	...
	InitFirstPhaseChunk(&chunk1_L);
	InitFirstPhaseChunk(&chunk1_R);

	InitSecondPhaseChunk(&chunk2_L);
	InitSecondPhaseChunk(&chunk2_R);

	memset(&chunk1_L, 0, sizeof(struct FirstPhaseChunk));
	memset(&chunk1_R, 0, sizeof(struct FirstPhaseChunk));

	memset(&chunk2_L, 0, sizeof(struct SecondPhaseChunk));
	memset(&chunk2_R, 0, sizeof(struct SecondPhaseChunk));
...
void process_audio(int* input_buf, int* output_buf, int nbytes)
{
   ...
   /* array size has to be updated */
   	uint8_t inputEncryption[IVlength + MAX_MESSAGE_LENGTH + SHA256_DIGEST_LENGTH] = {0};
   	uint8_t outputEncryption[IVlength + MAX_MESSAGE_LENGTH + SHA256_DIGEST_LENGTH] = {0};
   	uint8_t outputDecryption[IVlength + MAX_MESSAGE_LENGTH + SHA256_DIGEST_LENGTH] = {0};
   ...
   for (iter=0; iter<niters; iter++){
    	...
    	FirstPhase (&input_rot[iter*BUFFERSIZE], &chunk1_L, &chunk1_R, pCompressed_in);
    	//encode(&input_rot[iter*BUFFERSIZE], &encoderChunkLeft, &encoderChunkRight, encoded);
    }

   /* Encryption */
    from16to8(pCompressed_in, inputEncryption, PACKSIZE);
    packetLength[0] = 2*PACKSIZE;
    encryptHMAC(outputEncryption, packetLength, inputEncryption, key, encryptKeyLength);

   	/* Decryption */
   	decryptHMAC(outputDecryption, packetLength, outputEncryption, key, encryptKeyLength);
   	from8to16(outputDecryption, pCompressed_out, PACKSIZE);

   	/* Decoding */
    for (iter=0; iter<niters; iter++){
    	SecondPhase (pCompressed_in, &chunk2_L, &chunk2_R, &output_rot[iter*BUFFERSIZE]);
    	...
    	for (i = 0; i < nr_elements; i++) {
    		...
    		//pshort[i] = input_rot[(iter*BUFFERSIZE)+i];
    	}
    }
...
/*
	...
	printf("Plaintext before encryption: "); printCharNoSpaces(inputEncryption, BUFLEN*2);
	printf("\nCiphertext after encryption: "); printArrayNoSpaces(outputEncryption, BUFLEN*2 );
	printf("\nPlaintext after decryption: "); printCharNoSpaces(outputDecryption, BUFLEN*2);
#endif
*/
