...
***************************************************************************/
int wavpcm_input_read (struct wavpcm_input *input, short destBuffer[BUFFERSIZE*BUFFERS]) {
  ...
  int tempBufferSize = BUFFERSIZE*BUFFERS / 2;     /* how many temporal samples per buffer, regardless of channels */
  ...
  if (input->bitDepth == 8) {
    ...
    char tempBuffer[BUFFERSIZE*BUFFERS];
    readSize = fread(tempBuffer, 1, bytesToRead, input->fileHandle);

    for (destPos = 0; destPos<BUFFERSIZE*BUFFERS; destPos+=2) {
    ...
    /* We then need to read the data into that buffer. We don't check readSize here.
     * The reason is that we may have really found the end of the file, which is OK.  */
    short tempBuffer[BUFFERSIZE*BUFFERS];
    readSize = fread(tempBuffer, 1, bytesToRead, input->fileHandle);
...
***************************************************************************/
void wavpcm_output_write (struct wavpcm_output *output, short inputBuffer[BUFFERSIZE*BUFFERS], int bytesToWrite)
{
  /* Declare variables. */
  int curPos = 0;
