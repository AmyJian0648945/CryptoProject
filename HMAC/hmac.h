...
void concat(uint8_t* base, uint8_t* arrayToAppend, uint16_t baseLength, uint16_t lengthToAppend){
	...
	for(i=start; i<baseLength + lengthToAppend; i++){
...
void XOR(uint8_t* output, uint8_t* input1, uint8_t* input2, uint16_t lengthToXOR){
	uint16_t i = 0;
	for(i=0; i<lengthToXOR; i++) output[i] = input1[i] ^ input2[i];
}
...
void hmac(uint8_t* output, uint8_t* secretKey, uint8_t* inputMsg, uint16_t msgLength){
	uint8_t ipad[DATA_BLOCK_SIZE] = {0}, opad[DATA_BLOCK_SIZE] = {0};
	uint8_t paddedKey[DATA_BLOCK_SIZE] = {0};
	uint16_t i = 0;

	uint8_t result1[DATA_BLOCK_SIZE + msgLength] = {0};
	...
	for(i=0; i<SECRET_KEY_SIZE; i++) paddedKey[i] = secretKey[i];
	...
	concat(result1, inputMsg, DATA_BLOCK_SIZE, msgLength);
	simpleHashWithLength(output, result1, DATA_BLOCK_SIZE + msgLength); 
