...
*/
void modularInverse(uint32_t *x, uint32_t *y, uint32_t *inverse, uint16_t sizeX, uint16_t modLength){
		
		/* Extend x */
		uint32_t copyOfX[MAXLENGTH] = {0};
		uint32_t copyOfY[MAXLENGTH] = {0};
		uint32_t u[MAXLENGTH] = {0};
		uint32_t v[MAXLENGTH] = {0};
		uint32_t A[MAXLENGTH] = {0};
		uint32_t B[MAXLENGTH] = {0};
		uint32_t C[MAXLENGTH] = {0};
		uint32_t D[MAXLENGTH] = {0};
		uint16_t size = 0;
		uint16_t g = 0;
		int i;
		...
		for(i=0; i<size-modLength; i++)    copyOfY[i] = 0x00;
		for(i=size-modLength; i<size; i++) copyOfY[i] = y[i-(size-modLength)];
