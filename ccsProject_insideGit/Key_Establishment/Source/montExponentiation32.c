...
*/
void montMultiplication( uint32_t *x, uint32_t *y, uint32_t *m, uint32_t *result, uint16_t mInvLastBit, uint16_t sizeM, uint16_t sizeR, uint16_t extraVariableForR){
	...
	/* step 2 */
	/* n = sizeM*32; */
	n = (sizeR-1)*32 + extraVariableForR*16;
	for(i=0;i<n;i++){
		/* Step 2.1 */
...
void montExp( uint32_t *x, uint32_t *m, uint32_t *e, uint32_t *result, uint16_t sizeX, uint16_t sizeM, uint16_t sizeE){
	...
	one[sizeM-1] = 0x0001;
	montMultiplication(xExt,R2mod,m,xtilde,mInvLastBit,sizeM,sizeR,0);
	/* modMult(x,R,m,xtilde,sizeX,sizeM+1,sizeM); */
	for(i=t;i>=0;i--){
		montMultiplication(A,A,m,A,mInvLastBit,sizeM,sizeR,0);
		ei = (copyOfE[msbWord]>>(31-posWord))%2;
		if (ei == 1){
			montMultiplication(A,xtilde,m,A,mInvLastBit,sizeM,sizeR,0);
		}
		multiplyByTwo(copyOfE,sizeE);
	}
	montMultiplication(A,one,m,A,mInvLastBit,sizeM,sizeR,0);
