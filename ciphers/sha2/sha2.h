...
// See 	https://www.freebsd.org/cgi/man.cgi?query=SHA256_Init&sektion=3&apropos=0&manpath=FreeBSD+7.1-RELEASE 
// or 	https://www.mirbsd.org/htman/i386/man3/sha2.htm (gives example)
...
typedef struct _SHA256_CTX {
	...
	uint64_t	bitcount;	/* number of bytes processed  */
	uint8_t		buffer[SHA256_BLOCK_LENGTH];  /* data block being processed */
} SHA256_CTX;
...
/* Function definition
*	void SHA256_Init(); 	//initializes a SHA_CTX structure.
*	void SHA256_Update();	//can be called repeatedly with chunks of the 
								message to be hashed (len bytes at data)
*	void SHA256_Final();	//places the message digest in md, which must 
								have space for SHA_DIGEST_LENGTH == 20 bytes 
								of output, and erases the SHA_CTX.
*	char* SHA256_End();		//is a wrapper for SHA256_Final() which	converts 
								the return value to a 65-character (including
								the terminating	'\0') ASCII string which 
								represents the 256 bits in hexadecimal.
*	char* SHA256_Data();
*/
