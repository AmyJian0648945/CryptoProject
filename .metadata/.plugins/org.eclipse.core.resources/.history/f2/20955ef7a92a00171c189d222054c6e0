...
static void SHAtoByte(uint8_t *output, uint32_t *input, unsigned len);
...
static void SHSTransform( digest, data )
    ...
    D = digest[ 3 ];
    E = digest[ 4 ];
    memcpy( (void*)eData, (void*)data, SHS_DATASIZE );
...
static void longReverse(uint32_t *buffer, int byteCount)
{
    uint32_t value;
...
void SHAUpdate(SHA_CTX *shsInfo, uint8_t *buffer, int count)
{
    uint32_t tmp;
    int dataCount;
    ...
    while( count >= SHS_DATASIZE )
        {
        memcpy( (void*)shsInfo->data, (void*)buffer, SHS_DATASIZE );
        longReverse( shsInfo->data, SHS_DATASIZE);
        SHSTransform( shsInfo->digest, shsInfo->data );
        buffer += SHS_DATASIZE;
        count -= SHS_DATASIZE;
    ...
    memcpy( (void*)shsInfo->data, (void*)buffer, count );
    }

/* Final wrapup - pad to SHS_DATASIZE-byte boundary with the bit pattern
   1 0* (64-bit count of bits processed, MSB-first) */

void SHAFinal(uint8_t *output, SHA_CTX *shsInfo)
{
    int count;
    uint8_t *dataPtr;
    ...
    *dataPtr++ = 0x80;
    ...
    if( count < 8 )
        ...
        memset( (void*)shsInfo->data, 0, SHS_DATASIZE - 8 );
        }
    else
    ...
    SHSTransform( shsInfo->digest, shsInfo->data );
        ...
        memset((void*)shsInfo, 0, sizeof(shsInfo));
}
...
/*
  sha1 is the function that does the actual sha1-computation. the array
  <hashed> contains the sha1-hash value computed on <nofDataBytes> bytes in
  <data>.
*/
...
void sha1(uint8_t *hashResult, uint8_t *data, int nofDataBytes){
  ...
  SHAInit(&sha);
  SHAUpdate(&sha,data,nofDataBytes);
  SHAFinal(hashResult,&sha);
}
